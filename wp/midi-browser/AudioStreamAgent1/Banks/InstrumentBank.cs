...
namespace CSharpSynth.Banks
{
    public class InstrumentBank
    {
        ...
        public void loadBank(string bankfile)
        {
            ...
            loadStream(Application.GetResourceStream(new Uri(bankfile, UriKind.Relative)).Stream, bankfile.Substring(0, bankfile.LastIndexOf("/")), null, null);
        ...
        public void reload(int sampleRate)
        {
            this.SampleRate_ = sampleRate;
            List<byte> InstPrograms = new List<byte>();
            List<byte> DrumPrograms = new List<byte>();
            for (int x = 0; x < Bank.Count; x++)
            {
                if (Bank[x] != null)
                    InstPrograms.Add((byte)x);
            }
            for (int x = 0; x < DrumBank.Count; x++)
            {
                if (DrumBank[x] != null)
                    DrumPrograms.Add((byte)x);
            }
            loadBank(InstPrograms.ToArray(), DrumPrograms.ToArray());
            reCalculateMemoryUsage();
        }
        //--Public Properties
        ...
        private void changeSampleRate(int newSampleRate)
        {
            this.SampleRate_ = newSampleRate;
            for (int x = 0; x < Bank.Count; x++)
            {
                if(Bank[x] != null)
                    Bank[x].enforceSampleRate(newSampleRate);
            }
            for (int x = 0; x < DrumBank.Count; x++)
            {
                if (DrumBank[x] != null)
                    DrumBank[x].enforceSampleRate(newSampleRate);
            }
        }
