...
public class FireScreen extends GameCanvas implements Runnable
{
	...
	{
		Log.logInfo("soft key pressed.");
		if (k == leftSoftKey && current.leftSoftKeyCommand != null)
		{
			Log.logInfo("left soft key pressed.");
		...
		{
			Log.logInfo("right soft key pressed.");
			...
			Thread th = new Thread()
			{
				public void run()
				{
					Log.logInfo("trigger right soft key. current component id is ");
