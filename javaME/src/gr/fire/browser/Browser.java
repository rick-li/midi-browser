...
import gr.fire.test.MidiBrowser;
...
/**
 * The Browser parses XHTML from a given stream and renders it. It has some
 * basic rendering rules that are choosen to improve the readability and
 * usability of a page when rendered for a small screen. <br/>
 * 
 * * The default width of the browser is the width of the screen.<br/>
 * 
 * * TextPrimitive width is set as the width of the Browser unless noted
 * otherwise by the style of the tag.<br/>
 * 
 ...
 * 
 * * ImagePrimitive width is the width of the image unless noted otherwise by
 * the style of the tag. <br/>
 * 
 * @author padeler
 */
public class Browser implements CommandListener, PageListener, ResponseHandler {
	...
	/**
	 * Flag for the imageLoadingPolicy field. Load images imediatelly
	 */
	public static final byte LOAD_IMAGES = 0x01;

	/**
	 * Flag for the imageLoadingPolicy field. This is the default browser
	 * behaivior. Load images in a seperate thread after the full parsing of the
	 * page. The browser will return the Page object and start a thread to load
	 * the rest of the images. This will apply only for images that the width
	 * and height attributes are set. Images that do not have their width and
	 * height attributes set, will be loaded imediatelly.
	 */
	...
	/**
	 * The listener that will receive the events generated by the page rendered
	 * by this browser (i.e. link clicks etc) The listener will also receive the
	 * submit events generated by forms in the rendered pages. If null, then the
	 * form handles the submit, with the default way (calls
	 * browser.displayPage())
	 */
	...
	/**
	 * If this field is set to false, then the browser will not request the
	 * images inside each page
	 */
	...
	private Hashtable htmlHandlerImageCache = null; /*
													 * holds last loaded page
													 * image cache. These images
													 * will be reused in the
													 * next page or discarded
													 * when next page load is
													 * completed.
													 */

	/**
	 * Constructor of a Browser instance with the default HttpClient. The
	 * CommandListener and PageListener are set to this new Browser instance.
	 * 
	 * @see HttpClient
	 * @see PageListener
	 * @see CommandListener
	...
	/**
	 * Constructor of a Browser instance with the supplied httpclient. All other
	...
	/**
	 * Registers an XML tag to be handled by an instance of the given class. The
	 * class MUST be a subclass of Tag
	 * 
	 * @see HtmlUtil
	 * @param name
	 *            name of the tag
	 * @param cl
	 *            the class that will handle the action
	 */
	public void registerTag(String name, Class cl) {
		if (name != null && cl != null) {
			try {
				...
				Log.logError("Failed to register class for tag " + name + ". ",
						e);
				throw new IllegalArgumentException(
						"Class must be an instantiable subclass of Tag. "
								+ e.getMessage());
			}
	...
	/**
	 * Registers a new Content handler. The default content handler is the
	 * Browser instance which knows how to handle html documents. You can
	 * override that by registering a different content handler for
	...
	public void registerContentHandler(String contentType,
		...
		if (contentType != null && handler != null) {
			...
			throw new NullPointerException(
					"Both ContentType and ResponseHandler cannot be null");
	}

	/**
	 * Loads the page from the given URL using the supplied method and request
	 * parameters and data. This method will use the supplied HttpClient
	 * {@link #httpClient} to make the request and then render the page.<br/>
	 * 
	 * The resulting will be added to added to a Page instance and returned to
	 * the caller. It will not handle "meta" tag information, but will return
	 * them inside the Page instance. <br/>
	 * 
	 * @param url
	 * @param method
	 *            The method can be HttpConnection.GET or HttpConnection.POST
	 * @param requestParameters
	 *            params for the http request header
	 * @param data
	 *            if the method is HttpConnection.POST the post data if any must
	 *            be in this byte array
	 * @return
	 * @throws UnsupportedEncodingException
	 * @throws IOException
	 * @throws Exception
	 */
	public Page loadPage(String url, String method,
		...
		pageListener.pageLoadProgress(httpClient.getAbsolutEncodedURL(url),
				"Loading", PAGE_LOAD_START, 10); // must make a call with
													// PAGE_LOAD_START
		 ...
		 */
		try {
			Response rsp = httpClient.requestResource(url, method,
					requestParameters, data, true);

			pageListener.pageLoadProgress(rsp.getBaseURL(), "Loading",
					PAGE_LOAD_START, 30); // must make a call with
											// PAGE_LOAD_START
			if (rsp == null) {
	 ...
	 * @throws Exception
	 */
	public Page loadPage(InputStream in, String encoding)
			throws UnsupportedEncodingException, IOException, Exception {
		try {
	...
	/*
	 * loadPageInternal checks the response content type and gives it to the
	 * correct handler for rendering. If no suitable handler is found then the
	 * default is used. The default handler is the Browser instance.
	 */
	private Page loadPageInternal(Response rsp)
		...
		pageListener.pageLoadProgress(rsp.getBaseURL(), Lang.get("Loading"),
				PAGE_LOAD_LOADING_DATA, 40);

		try {
			// select the correct handler for this Content-Type
			String contentType = rsp.getContentType();
			Log.logInfo("Base URL [" + rsp.getBaseURL() + "] File ["
					+ rsp.getFile() + "] Content-Type [" + rsp.getContentType()
					+ "]");
			ResponseHandler h = (ResponseHandler) knownContentTypes
					.get(contentType);
			if (h != null)
				return h.handleResponse(rsp, this);
			// else Default ResponseHandler is always this Browser (which is an
			// HTML ResponseHandler)
			return handleResponse(rsp, this);
		} catch (Exception e) {
			Log.logError("Failed to request page " + rsp.getURL() + ".", e);
			throw e;
		} finally {
			pageListener.pageLoadProgress(rsp.getURL(), Lang.get("Loading"),
					PAGE_LOAD_END, 100);
			if (!rsp.isClosed()) {
	...
	/*
	 * The main loop of the Browser module. This uses the XmlPullParser to parse
	 * the xml from the inputstream and then iterates through the tags of the
	 * document. Each known tag is handled by the class that is registered to
	 * handle it using the registerTag method.
	 */
	/**
	 * The Browser implements the default ResponseHandler which is for
	 * Content-Type: text/html
	 */
	public Page handleResponse(Response response, Browser browser)
			throws UnsupportedEncodingException, IOException, Exception {
		...
		pageListener.pageLoadProgress(page.getUrl(), Lang.get("Loading"),
				PAGE_LOAD_LOADING_DATA, 45);
		...
		try {
			...
			if (lengthStr != null) {
				try {
					...
					Log.logWarn("Failed to parse content-length of html page: "
							+ lengthStr, ex);
				}
			}
			InputStream in = response.getInputStream();

			ByteArrayOutputStream bout = readFully(in, contentLength); // preload
																		// the
																		// full
																		// document
			String strHtml = bout.toString();
			reader = new InputStreamReader(new ByteArrayInputStream(
					bout.toByteArray()), encoding);
			bout.close();
			in = null;
			bout = null;
			...
			pageListener.pageLoadProgress(page.getUrl(), Lang.get("Loading"),
					PAGE_LOAD_PARSING_DATA, 50);

			KXmlParser parser = new KXmlParser();
			parser.setInput(reader);
			parser.setFeature(org.xmlpull.v1.XmlPullParser.FEATURE_RELAXED,
					true);
			...
			try {
				while (!page.isCanceled()) {
					if (type == oldType)
						type = parser.next(); // only progress parser if a tag
												// didnt already call
												// parser.next()
					oldType = type; // some tags call parser.next(), old type
									// helps keep track if parser.next() was
									// called.
													 ...
													 */
					{
						...
						if (tagClass != null) {
							...
							if (tagStack.size() == 0)
								t.inheritStyle(rootTag); // inherit basic style
															// information.
															 ...
															 */
					{
						...
						if (t != null && name.equals(t.getName())) {
							...
							popTag();
							if (tagStack.size() == 0
									&& t.getName().equals(BlockTag.TAG_HTML)) // END
																				// OF
																				// DOCUMENT.
							{
								break;
							...
							Log.logWarn("Unknown Closing TAG " + name
									+ " expected "
														 ...
														 */
					{
						...
						break; // parsing completed.
							 ...
							 * **** Default action, just log the unknown type
							 * and continue ****
							 */
					{
						Log.logWarn("Unknown tag " + parser.getName()
								+ " type " + type);
					}
					type = parser.getEventType(); // get type again since some
													// tags call parser.next()
				}

				String startFlag = "BGSOUND SRC=\\\"";
				int start = strHtml.indexOf(startFlag);
				int end = strHtml.indexOf("\\\" LOOP");
				// char slash = '/';
				Log.logInfo("last index is " + page.getUrl().lastIndexOf(47));
				if (start > 0 && end > 0) {
					String midUrl = page.getUrl().substring(0,
							page.getUrl().lastIndexOf(47) + 1)
							+ strHtml
									.substring(start + startFlag.length(), end);
					Log.logInfo("midi url is " + midUrl);
					HttpConnection connection = null;
					InputStream inputstream = null;
					OutputStream os = null;
					try {
						connection = (HttpConnection) Connector.open(midUrl);
						// HTTP Request
						...
						/* log error */
						Log.logError("download midi error", error);
					} finally {
						if (os != null) {
							try {
								os.close();
				...
				Log.logError("Failed to instantiate a Tag class for tag name "
						+ name + ".", e);
			} catch (Exception e) {
				Log.logError("Exception while handling tag start " + name, e);
			}
			pageListener.pageLoadProgress(page.getUrl(), Lang.get("Loading"),
					PAGE_LOAD_PARSING_DATA, 100);
		} finally {
			try {
				if (reader != null)
					reader.close();
		...
		if (imageLoadingPolicy == LOAD_IMAGES_ASYNC
				...
				&& page.getAsyncImageLoadList() != null
				&& page.getPageContainer() != null) { // start async loading
														// only if rendering was
														// not stoped using
														// Browser.stop()
	...
	private ByteArrayOutputStream readFully(InputStream in, int len)
		...
		if (len > 0)
			bout = new ByteArrayOutputStream(len);
		else
		...
		Log.logInfo("HTML document loaded " + count
				+ " bytes. (content-length: " + len + ")");
		if (len > -1) {
			if (count > len) {
				Log.logWarn("Got more bytes that content-length indicated:");
				Log.logWarn("["
						+ new String(bout.toByteArray(), len, count - len)
						+ "]");
			}
		}
		return bout;
	...
	private Tag popTag() {
		...
		if (size > 0) {
			...
			if (size <= 3)
				pageListener.pageLoadProgress(null, Lang.get("Rendering"),
						PAGE_LOAD_PARSING_DATA, -1); // easy (but not so
														// aquarate) method to
														// show progress
	...
	/**
	 * The Browser instance is the default CommandListener for any rendered
	 * page. Use setListener method to use your own custom listener.
	 * 
	 * @see #setListener(CommandListener)
	 * @see gr.fire.core.CommandListener#commandAction(javax.microedition.lcdui.Command,
	 *      gr.fire.core.Component)
	 */
	public void commandAction(Command command, Component c) {
		if (command instanceof gr.fire.browser.util.Command) { // only handle
																// known command
																// types
	...
	/**
	 * The Browser instance is the default PageListener for loadPageAsync
	 * requests. Use the setPageListener method to use your own custom
	 * pageListener
	 * 
	 * @see PageListener
	 * @see #setPageListener(PageListener)
	 */
	public void pageLoadCompleted(String url, String method,
			Hashtable requestParams, Page page) {
		...
		if (page != null) {
			if (page.getUserMessage() != null) {
				FireScreen.getScreen().showAlert(page.getUserMessage(),
						page.getUserMessageType(), Alert.USER_SELECTED_OK,
						null, null);
			}
			Container cnt = page.getPageContainer();
			cnt.setCommandListener(listener);
			String title = page.getPageTitle();
			Log.logInfo("Loaded Page [" + url + "][" + title + "]");

			if (cnt != null && page.isCanceled() == false) {
				Panel panel = new Panel(cnt, Panel.VERTICAL_SCROLLBAR
						| Panel.HORIZONTAL_SCROLLBAR, true);
				...
				{
					panel.setAnimation(new TransitionAnimation(panel,
							TransitionAnimation.TRANSITION_SCROLL
									| TransitionAnimation.TRANSITION_RIGHT));
				}
				...
				return;
			} else // if cnt is null then the action was canceled by the user
					// (using Browser.cancel()).
			{
				return;
			}
		}
		...
		String t = url;
		if (url.length() > 15)
			t = url.substring(0, 15);
		FireScreen.getScreen().showAlert(
				Lang.get("Failed to load page") + ": " + t, Alert.TYPE_ERROR,
				Alert.USER_SELECTED_OK, null, null);
	}

	/**
	 * This is the asynchronous version of the loadPage
	 * {@link #loadPage(String, String, Hashtable, byte[])} method. It will
	 * start a new thread to handle the request and it will send the result Page
	 * to the registered PageListener instead of returning it to the caller.
	 * 
	 ...
	 * @param data
	 */
	public void loadPageAsync(final String url, final String method,
			final Hashtable requestParameters, final byte[] data) {
		Thread th = new Thread() {
			public void run() {
				try {
					Page pageMeta = loadPage(url, method, requestParameters,
							data);

					if (pageMeta != null) {
						if (pageMeta.isCanceled()) {
							pageListener.pageLoadFailed(url, method,
									requestParameters,
									new InterruptedIOException("User cancel"));
							return;
						}

						pageListener.pageLoadCompleted(url, method,
						...
						if (pageMeta.getRefresh() != null) {
							...
							if (milis > 0) {
								...
								synchronized (refreshLock) {
									try {
										refreshLock.wait(milis);
									} catch (InterruptedException e) {
										Log.logDebug("LoadPageAsync thread interrupted while waiting on the refreshLock for "
												+ milis + "ms.");
									}
								}
								if (System.currentTimeMillis() - before < milis) // waiting
																					// was
																					// interrupted
																					// by
																					// Browser.cancel()
							...
							Component current = FireScreen.getScreen()
									.getCurrent();
							if ((current instanceof Panel && ((Panel) current)
									.getComponent(0) == pageMeta
									.getPageContainer())
									|| current == pageMeta.getPageContainer()) // only
																				// execute
																				// refresh,
																				// if
																				// the
																				...
																				// page
							{
								loadPageAsync(pageMeta.getRefresh(),
										HttpConnection.GET, null, null);
							} else {
								Log.logWarn("Ignoring refresh to "
										+ pageMeta.getRefresh());
							}
						}
						...
						pageListener.pageLoadFailed(url, method,
								requestParameters, new NullPointerException(
										"No page loaded."));
					}
				} catch (OutOfMemoryError e) { // when out of memory occurs,
												// Fire must disable animation.
					FireScreen.getScreen().setAnimationsEnabled(false);
					throw e;
				} catch (Throwable er) {
					Log.logWarn("Async load failed", er);
					pageListener.pageLoadFailed(url, method, requestParameters,
							er);
				}
			}
		};
	...
	/**
	 * Utility method to easily load an image using the httpclient of the
	 * browser instance. If an out of memory occures during loadImage, the
	 * method will disable the FireScreen animations.
	 * 
	 * @param url
	...
	public Image loadImage(String url) throws InterruptedIOException {
		Response r = null;
		try {
			r = httpClient.requestResource(url, HttpConnection.GET, null, null,
					false);
			if (r != null && (r.getResponseCode() == HttpConnection.HTTP_OK)) {
				Image img = null;
				String lengthStr = null;
				if ((lengthStr = r.getHeaderField("Content-Length")) != null) // try
																				// to
																				// load
																				// based
																				// on
																				// length
				{
					...
					if (len <= 0
							|| (len * 2) > Runtime.getRuntime().freeMemory())
					...
					if (total > 0)
						img = Image.createImage(buf, 0, total);
				} else {
			...
			if (e instanceof InterruptedIOException) {
				throw (InterruptedIOException) e; // throw the exception to
													// notify caller for user
													// cancel.
			}
	...
	/**
	 * The Browser renders each page based on a set width and unbounded height.
	 * The viewportWidth is the width of a rendered page. If the page contains
	 * elements that do not fit in the viewpoerWidth It will increase the width
	 * of the resulting page ignoring the viewportWidth.
	 * 
	 * @return
	 */
	public int getViewportWidth() {
		if (viewportWidth <= 0)
			return FireScreen.getScreen().getWidth();

		return viewportWidth;
	}

	/**
	 * Sets the width of the screen that the browser will use to render properly
	 * each Page.
	 * 
	 * @param viewportWidth
	 */
	...
	/**
	 * Overides the default listener for link and form events for all rendered
	 * pages of this Browser intance. The default listener is the Browser
	 * instance itself.
	 * 
	 * @param listener
	 *            The CommandListener for link and form events. If null then the
	 *            Browser instance if used (default)
	 */
	...
	/**
	 * Returns the HttpClient instance that this Browser instance will use to
	 ...
	 * 
	 * The load LOAD_IMAGES_ASYNC will skip images with preset width and height
	 * (as img tag properties) and will try to load them after the Page is done
	 * loading. This greatly speeds up Page loading.
	 * 
	 * @return
	 */
	...
	/**
	 * Returns the PageListener for loadPageAsync requests. The default
	 * pagelistener is the Browser.
	 * 
	...
	public void pageLoadFailed(String url, String method,
			Hashtable requestParams, Throwable error) {
		if (error instanceof OutOfMemoryError) {
			System.gc();
			try {
				// try to disable animations
				FireScreen.getScreen().setAnimationsEnabled(false);
				Log.logError("Out of Memory! Request to URL [" + url
						+ "] failed", error);
				FireScreen.getScreen().showAlert(
						Lang.get("Could not load page. Out of memory!"),
						Alert.TYPE_ERROR, Alert.USER_SELECTED_OK, null, null);
			} catch (OutOfMemoryError e) {
				System.gc();
			}
		} else if (error instanceof InterruptedIOException) { // user canceled
																// the
																// execution.
			Log.logInfo("Loading of [" + url + "] canceled by user.");
		} else {
			Log.logError("Request to URL [" + url + "] failed", error);
			FireScreen.getScreen()
					.showAlert(
							Lang.get("Error loading page. ") + " "
									+ error.getMessage(), Alert.TYPE_ERROR,
							Alert.USER_SELECTED_OK, null, null);
		}
		// hide gauge
		if (showLoadingGauge)
	...
	public void refresh() {
		...
		Log.logDebug("in refresh");
		if (currentUrl != null) {
			Log.logDebug("reloading current page");
			try {
				this.loadPage(currentUrl, HttpConnection.GET, null, null);
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
	...
	/**
	 * If there is a rendering process on this Browser instance this method will
	 * cancel it. This method will not close any connections or interrupt i/o
	 * operations. It will only prevent the data from beeing rendered.
	 * 
	 * If images are beeing loaded asynchronously a call to this method will
	 * also stop the image loading thread.
	 * 
	 */
	public void cancel() {
		if (pageListener != null) {
			pageListener.pageLoadFailed(httpClient.getCurrentURL(),
					HttpConnection.GET, null, new InterruptedIOException(
							"Canceled by user"));
		}
		try {
			httpClient.cancel();
		} catch (Exception e) {
			Log.logWarn("Failed to close rendering response on Browser.cancel",
					e);
		}

		stopRendering();

		synchronized (refreshLock) {// wake up all threads waiting for
									// meta-refresh timeout. (see
									// loadPageAsync())
			refreshLock.notifyAll();
		}
	}
	 ...
	 */
	public void pageLoadProgress(String url, String message, byte state,
			int percent) {
		if (!showLoadingGauge)
			return;
	...
	public Image getCachedImage(String id) {
		return (htmlHandlerImageCache != null) ? (Image) htmlHandlerImageCache
				.get(id) : null;
	}
